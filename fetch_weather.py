...
def main():
    ...
    # 3. Optional: Fetch Township Weather Forecast (F-D0047-091)
    if args.fetch_townships:
        print("\n[Step 2/2] Fetching Township Weather Forecast (F-D0047-091)...")
        try:
    ...
    try:
        ...
        if not df.empty:
            ...
            print(df[avail_cols].head(10).to_string(index=False))
            print(f"\nTotal Records in Database: {len(df)}")
        else:
            print("No records currently stored in database.")
    except Exception as err:
